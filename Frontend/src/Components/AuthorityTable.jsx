...
const AuthorityTable = () => {
  ...
  const [authority, setAuthority] = useState('URBAN');
  ...
  const fetchComplaints = async () => {
    ...
    try {
      const q = query(
        ...
        // where('assignedAuthority', '==', `${auth.currentUser.uid}`)
      );
  ...
  return (
    <div>
      ...
      <div className="center-text">
        <p className="big-font">Municipal Council</p>
      </div>

      <div className="authority-table-container">
        <div>
          <AuthoritySidebar route={'/AuthorityTable'} />
        </div>
...
const Table1 = ({
  ...
  const assignComplaints = async () => {
    ...
    try {
      // Update the complaints for the authority user
